const sentences = ["The quick brown fox jumps over the lazy dog.Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quia atque perspiciatis impedit recusandae soluta error fugiat labore, omnis vero eligendi laborum tempora harum incidunt aspernatur quibusdam odit iusto nam ipsum.",
...
function disableCheating() {
  ...
  inputEl.addEventListener("keydown", function (e) {
    ...
    if (e.ctrlKey && ["v", "c", "x"].includes(e.key.toLowerCase())) {
      e.preventDefault();
    }
  });
...
function score() {
  ...
  for (let i = 0; i < typed.length; i++) {
    if (typed[i] === sentencez[i]) correct++;
  }
...
function startGame() {
  ...
  // Pick new sentence each time
  const randomIndex = Math.floor(Math.random() * sentences.length);
  sentencez = sentences[randomIndex];
  ...
  totalSeconds = parseInt(options.value) * 60;
